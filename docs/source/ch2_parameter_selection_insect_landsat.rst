...
其中 :math:`T` 是一个转换矩阵。对于每个波段，``states`` :math:`a` 包含三个随时间变化的元素：趋势、年际、半年际和三年际（如果可用）。这些 ``states`` 支持对季节性、长期趋势和其他动态进行时间段内分析，从而补充了 S-CCD 检测到的光谱断点。我们将在第 5 课中详细介绍 ``state analysis``。
