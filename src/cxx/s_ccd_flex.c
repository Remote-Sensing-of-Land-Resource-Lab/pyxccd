    ...
    if (b_anomaly == TRUE)
    {
        ...
        { // meaning that over 3 anomaly pixel
            ...
            // if ((*num_fc_anomaly == 0) | (clrx[cur_i] - rec_cg_anomaly[*num_fc_anomaly - 1].t_break > AVE_DAYS_IN_A_YEAR))// must has a gap of 1 year with the last anomaly break
            {
                ...
                {
                    v_diff_tmp = (float **)allocate_2d_array(nbands, anomaly_conse, sizeof(float)); // used to calculate angle
                    v_dif_mag_tmp = (float **)allocate_2d_array(nbands, anomaly_conse,
                                                                sizeof(float));
                    for (i_b = 0; i_b < nbands; i_b++)
                    {
                    ...
                    mean_angle_anomaly = MeanAngl_float(v_diff_tmp, nbands, anomaly_conse) * 100;
    ...
    double lambda)
{
    ...
    if (*nrt_mode % 10 == NRT_MONITOR_STANDARD)
    {
        ...
        if (num_obs_processed > 0)
        {
            ...
            {
                v_diff_tmp = (float **)allocate_2d_array(nbands, anomaly_conse, sizeof(float));
                v_dif_mag_tmp = (float **)allocate_2d_array(nbands, anomaly_conse,
                                                            sizeof(float));
                for (i_b = 0; i_b < nbands; i_b++)
                {
                ...
                // if (v_dif_mag_norm[anomaly_conse - 1] <= DEFAULT_COLD_TCG)
                {
                    ...
                    if (anomaly_conse > 1)
                    {
                        ...
                        if (min_cm > MAX_SHORT)
                            min_cm = MAX_SHORT;
                        // mean_angle = angl_scatter_measure(medium_v_dif, v_diff_tmp, NUM_LASSO_BANDS, anomaly_conse, lasso_blist_sccd);
                        mean_angle_scale100 = MeanAngl_float(v_diff_tmp, nbands, anomaly_conse - 1) * 100;
                ...
                else
                {
                    ...
                    { // all observation in the last conse passed T_MIN_CG_SCCD
                        ...
                        if (min_cm > MAX_SHORT)
                            min_cm = MAX_SHORT;
                        mean_angle_scale100 = MeanAngl_float(v_diff_tmp, nbands, anomaly_conse) * 100;
                        if (mean_angle_scale100 > MAX_SHORT)
                            mean_angle_scale100 = MAX_SHORT;
