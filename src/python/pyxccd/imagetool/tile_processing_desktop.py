...
def block_tile_processing(
    ...
    # 检查是否已完成处理
    finished_file = "COLD_block{}_finished.txt".format(block_id) if method == "COLD" else "SCCD_block{}_finished.txt".format(block_id)
        ...
        try:
            ...
            print(
                "SCCD processing failed for block_x{}_y{}: {} ({})".format(
                    block_x, block_y, str(e), datetime.now(TZ).strftime("%Y-%m-%d %H:%M:%S")
                )
            )
...
def main(
    ...
    partial_func = functools.partial(
        ...
        probability_threshold,
        conse,
        True,  # b_c2 parameter set to True
        low_datebound,
        upper_datebound,
    )
