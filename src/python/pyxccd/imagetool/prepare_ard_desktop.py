...
def safe_stack_single_image_hls(
    ...
    logger: Logger,
    folder: str,
    low_date_bound: str = None,
    upp_date_bound: str = None,
) -> bool:
    """Process single HLS image stacking"""
    [_, _, _, imagetime, _, _] = folder.rsplit(".")
    img_date = dt.datetime(int(imagetime[0:4]), 1, 1) + dt.timedelta(int(imagetime[4:7]) - 1)
...
@click.command()
@click.option("--source_path",  help="HLS data directory")
@click.option("--yaml_path",  help="Configuration file path")
@click.option("--rank", default=1, help="Current process rank")
@click.option("--n_cores", default=1, help="Parallel cores number")
@click.option("--out_path",  help="Output directory")
@click.option("--low_date_bound", default="2019-1-1", help="Start date")
@click.option("--upp_date_bound", default="2024-12-31", help="End date")
...
def main(
    ...
    if exists(source_path):
        folder_list = [join(source_path, f) for f in os.listdir(source_path) 
