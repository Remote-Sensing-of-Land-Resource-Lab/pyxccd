...
def _export_map_processing(
    ...
    try:
        if not os.path.exists(join(result_path, filename)):
            print(f"[Warning] The input file {join(result_path, filename)} does not exist")
            raise FileNotFoundError(f"Input file not found: {join(result_path, filename)}")
        ...
        if method == "SCCDOFFLINE":
            ...
            for count, curve in enumerate(cold_block):
                ...
                if (curve["change_prob"] < 100 or curve["t_break"] == 0 or 
                    count == (len(cold_block) - 1)):
                    continue
        ...
        print(f"[Critical Error] Exception occurred while processing block {iblock+1}: {str(e)}")
...
@click.option(
    "--source_dir",
    type=str,
    help="Path to HLS tile directory"
)
@click.option("--n_cores", type=int, default=12, help="Number of CPU cores to use")
@click.option(
    "--result_path", 
    type=str,
...
@click.option(
    "--out_path",
    type=str,
    help="Output directory for saving maps"
)
@click.option(
    "--method",
    type=click.Choice(["COLD", "SCCDOFFLINE"]),
    default="COLD",
    help="Change detection algorithm used"
)
@click.option(
    "--yaml_path",
    type=str,
...
def main(
    ...
    if coefs is not None:
        try:
            ...
            raise ValueError("Illegal coefs inputs: example, --coefs='a0, c1, a1, b1, a2, b2, a3, b3, cv, rmse'")

        try:
            coefs_bands = [int(b.strip()) for b in coefs_bands.split(",")]
            assert all(elem in band_names for elem in coefs_bands)
        except:
            raise ValueError("Illegal coefs_bands inputs: example, --coefs_bands='0, 1, 2, 3, 4, 5, 6'")
    ...
    for year in range(year_lowbound, year_uppbound + 1):
        ...
        if coefs is not None:
            ...
            coef_results = np.vstack([np.hstack([
            ...
            for band_idx, band_name in enumerate(coefs_bands):
                for coef_index, coef in enumerate(coefs):
                    ...
                    with rasterio.open(
                        ...
                        dst.write(
                            coef_results[:profile["height"], :profile["width"], ninput],
                            1
                        )
                    ninput += 1
