...
reccg_dt = numpy.dtype(
    [
        ...
        (
            "category",
            numpy.short,
        ),  # the quality of the model estimation as a two-digit number (what model is used, what process is used)
        # first digit:
        ...
        (
            ...
            numpy.float32,
            (7, 8),
        ),  # coefficients for each time series model for seven spectral band, seven bands follow the order of "blue, green, red, nir, swir1, swir2, thermal"
        # seven row has 8 coefficients representing a 'annual-semiannual-trimode' harmonic model
        (
            "rmse",
            numpy.float32,
            7,
        ),  # RMSE for each time series model for each seven band
        (
            "magnitude",
            numpy.float32,
            7,
        ),  # the magnitude of difference between model prediction and observation for each spectral band
    ]
)
...
sccd_dt = numpy.dtype(
    [
        (
            "t_start",
            numpy.int32,
        ),  # ordenal date for the start of the time-series segment
        (
            "t_break",
            numpy.int32,
        ),  # ordenal date for the break of the time-series segment
        (
            "num_obs",
            numpy.int32,
        ),  # the number of "good" observations used for model estimation
        (
            ...
            numpy.float32,
            (NRT_BAND, SCCD_NUM_C),
        ),  # coefficients for each time series model for six spectral band
        (
            "rmse",
            numpy.float32,
            NRT_BAND,
...
nrtmodel_dt = numpy.dtype(
    [
        (
            "t_start_since1982",
            numpy.short,
        ),  # the date number since 1982-1-1 for the start of the time-series segment, equal to ordinal date + 723546
        (
            "num_obs",
            numpy.short,
        ),  # the number of "good" observations used for model estimation
        (
            "obs",
            numpy.short,
            (NRT_BAND, SCCD_CONSE_OUTPUT),
        ),  # eight multispectral observations at tail (6 * 8)
        (
            "obs_date_since1982",
            numpy.short,
            SCCD_CONSE_OUTPUT,
        ),  # eight observation dates (counted since 1982-1-1) at tail (6 * 8)
        (
            "covariance",
            numpy.float32,
            (NRT_BAND, 36),
        ),  # covariance matrix for six bands (6 * 36)
        (
            "nrt_coefs",
            numpy.float32,
            (NRT_BAND, SCCD_NUM_C),
        ),  # the current nrt_coefs (6 * 6)
...
reccg_dt_flex = numpy.dtype(
    [
        ...
        (
            ...
            numpy.float32,
            TOTAL_BAND_FLEX,
        ),  # RMSE for each time series model for each spectral band
